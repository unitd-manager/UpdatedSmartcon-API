...
exports.removeFilesByMediaRecord = (req, res) => {
  
  let select_sql = `SELECT * FROM media WHERE  media_id=${db.escape(req.body.media_id)}`;
  let query = db.query(select_sql,(err, result) => {

    if(result.length > 0){
        
         let delete_sql = `DELETE FROM media WHERE  media_id=${db.escape(req.body.media_id)}`;
    ...
    //   const filePath = 'storage/uploads/' + file.file_name;
    ...
    //       });
    //     } else {
    //       let delete_sql = `DELETE FROM media WHERE media_id=${db.escape(
    //         file.media_id
    //       )}`;
    //       let query = db.query(delete_sql, (err, result) => {
    //         if (err) {
